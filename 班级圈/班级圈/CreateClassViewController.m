...
@interface CreateClassViewController () <UIPickerViewDelegate,UIPickerViewDataSource,UITextFieldDelegate>
...
@property (nonatomic,retain)School* teachingSchool;
@property (nonatomic,retain)NSNumber* selectedSchoolId;
@end
...
{
    [self.txClassName resignFirstResponder];
    
    return NO;
}

-(void)uploadCreateOrganization
{
          ...
          success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              ...
              if ([status isEqualToNumber:[NSNumber numberWithInteger:0]]) {
                  ...
                  if ([[responseObject objectForKey:@"flag"] isEqualToNumber:[NSNumber numberWithInteger:0]]) {
                      ...
                      [self presentViewController:alertController animated:YES completion:nil];
                  } else if ([[responseObject objectForKey:@"flag"] isEqualToNumber:[NSNumber numberWithInteger:-1]]){
                      UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"创建失败！"
                                                                                               message:@"您还未认证教师"
                                                                                        preferredStyle:UIAlertControllerStyleAlert ];
                      
                      //添加取消到UIAlertController中
                      UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"去认证" style:UIAlertActionStyleDefault handler:^(UIAlertAction* action) {
                      ...
                      //添加取消到UIAlertController中
                      UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"重新认证" style:UIAlertActionStyleDefault handler:^(UIAlertAction* action) {
                          TeacherConfirmViewController* confirmVC = [[TeacherConfirmViewController alloc] init];
                          [self.navigationController pushViewController:confirmVC animated:true];
                      }];
                      [alertController addAction:cancelAction];
                  ...
                  UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"创建失败！"
                                                                                           message:@"您没有权限创建该班级，请选择重新认证或检查班级信息是否与认证时填写的相同"
                                                                                    preferredStyle:UIAlertControllerStyleAlert ];
                  //添加确定到UIAlertController中
                  UIAlertAction *OKAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:nil];
                  [alertController addAction:OKAction];
...
-(void)didTouchBtn: (UIButton* )sender
{
    ...
    if (sender.tag == self.createClass.tag) {
        NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
        if ([[defaults objectForKey:@"teacherFlag"] isEqualToNumber:[NSNumber numberWithInteger:0]]) {
            if (self.leftBtn.selected) {
                [self uploadCreateClass];
            } else if (self.rightBtn.selected ) {
                [self uploadCreateOrganization];
            }
        } else {
            UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"您还未认证教师，需要认证后才能进行学校班级创建" message:@"现在您只能创建培训机构的班级" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)createUI
{
    ...
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    NSNumber* schoolFlag = [defaults objectForKey:@"teacherFlag"];
    if ([schoolFlag isEqualToNumber:[NSNumber numberWithInteger:0]]) {
        self.cityView.value = [defaults objectForKey:@"schoolCity"];
        self.areaView.value = [defaults objectForKey:@"schoolArea"];
        self.kindView.value = [self.classifyArray objectAtIndex:[[defaults objectForKey:@"schoolClass"] integerValue]];
        self.organizeView.value = [defaults objectForKey:@"schoolName"];
        self.selectedSchoolId = [defaults objectForKey:@"schoolId"];
    ...
    self.txClassName.delegate = self;
    
    self.createClass = [UIButton new];
    [self.createClass setTitle:@"创建班级" forState:UIControlStateNormal];
    [self.createClass setBackgroundColor:[GlobalVar themeColorGetter]];
    self.createClass.layer.cornerRadius = 5;
    [self.createClass setTag:10011];
    [self.createClass addTarget:self action:@selector(didTouchBtn:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.createClass];
    [self.createClass mas_makeConstraints:^(MASConstraintMaker *make) {
...
//    
//    self.provinceArray = [self.regionDict allKeys];
